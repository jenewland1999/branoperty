...
import branoperty.model.user.UserAdmin;
import branoperty.util.FileHandler;
    ...
    private Session getSession(String username, String password, boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            if (userAdminUsername.equalsIgnoreCase(username) && userAdminPassword.equals(password)) {
                return new Session((UserAdmin) FileHandler.readObjFromFile(getAdminFile()));
            } else {
                return null;
            }
        }
